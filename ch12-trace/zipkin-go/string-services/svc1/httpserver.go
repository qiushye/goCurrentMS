...
func NewHTTPHandler(tracer *zipkin.Tracer, service Service) http.Handler {
	...
	// option := zipkinHttp.New
	concatHandler = zipkinHttp.NewServerMiddleware(tracer, zipkinHttp.SpanName("Concat"))(concatHandler)

	// Create the Sum handler.
	var sumHandler http.Handler
	sumHandler = http.HandlerFunc(svc.sumHandler)
	concatHandler = zipkinHttp.NewServerMiddleware(tracer, zipkinHttp.SpanName("Sum"))(concatHandler)
