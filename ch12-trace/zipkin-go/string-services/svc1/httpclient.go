...
type client struct {
	...
	traceRequest zipkinHttp.RequestSamplerFunc
}
...
func (c *client) Concat(ctx context.Context, a, b string) (string, error) {
	...
	// c.traceRequest(req.WithContext(ctx))
...
// transport.
func NewHTTPClient(tracer zipkin.Tracer, baseURL string) Service {
	return &client{
		baseURL:    baseURL,
		httpClient: &http.Client{},
		tracer:     tracer,
		// traceRequest: zipkinHttp.(tracer),
	}
}
